...
CREATE TABLE `tbl_auditoria_sesion` (
  ...
  `Estado` enum('SUCCESS','FAILED') DEFAULT NULL,
  PRIMARY KEY (`ID_Auditoria`),
...
INSERT INTO `tbl_datos_adicionales` VALUES ('D1019652358','daniel@gmail.com','3004506985','Padre',1,'1019652358',1,1,1,1,'ACTIVE'),('D1268945650','admin@panyfrut.com','3213397584','padre',1,'1268945650',1,1,1,1,'ACTIVE');
...
INSERT INTO `tbl_persona` VALUES ('1019652358','Daniel','Esteban','Pinto','Nu√±ez','2000-01-01','ACTIVE'),('1268945650','Dan','Den','Din','Don','2000-01-01','ACTIVE');
...
INSERT INTO `tbl_usuario` VALUES ('U1019652358','Esteban Pinto',_binary '0x89504E47',_binary '0x89504E470D0A1A0A000000',NULL,NULL,'2026-03-21 03:25:23','INACTIVE',1,'1019652358',2,'ACTIVE'),('U1268945650','admin@panyfrut.com',_binary '{ÿúÄ^N@>\ﬁ^Æ\ V˚\Ÿ',_binary '0+≤ìß\·≠Y\„\›\Ô\«\…#\r\Â˛ÆFV(\„I˚1\Î\Í\Ìπ',NULL,NULL,'2026-03-21 04:17:30','INACTIVE',1,'1268945650',2,'ACTIVE');
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_tbl_usuario_validar_login`(
    IN p_Nombre_Usuario VARCHAR(50),
    IN p_password VARCHAR(255),
    IN p_pepper VARCHAR(255)
)
BEGIN
    DECLARE v_salt VARBINARY(16);
    DECLARE v_hash_guardado VARBINARY(32);
    DECLARE v_hash_ingresado VARBINARY(32);
    DECLARE v_estado ENUM('ACTIVE','INACTIVE');
    DECLARE v_intentos INT;

    SELECT Password_Salt, Contrase√±a_Hash, Estado_Usuario, Intentos_Fallidos 
    INTO v_salt, v_hash_guardado, v_estado, v_intentos
    FROM TBL_USUARIO 
    WHERE Nombre_Usuario = p_Nombre_Usuario;

    IF v_salt IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Usuario no encontrado';
    ELSEIF v_estado = 'INACTIVE' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cuenta desactivada';
    ELSEIF v_intentos >= 5 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Cuenta bloqueada por exceso de intentos';
    ELSE
        SET v_hash_ingresado = UNHEX(SHA2(CONCAT(v_salt, p_password, p_pepper), 256));
        IF v_hash_ingresado = v_hash_guardado THEN
            ...
            SELECT ID_Usuario, Nombre_Usuario, FK_ID_Rol, Doble_Factor_Activo 
            FROM TBL_USUARIO WHERE Nombre_Usuario = p_Nombre_Usuario;
        ELSE
            UPDATE TBL_USUARIO SET Intentos_Fallidos = Intentos_Fallidos + 1 
            WHERE Nombre_Usuario = p_Nombre_Usuario;
...
-- Dump completed on 2026-03-23 14:52:03
