# CODIGOS FUNCIONALES PERO REMPLAZADOS POR UNA VERSIÓN MEJORADA
    # GUARDA AQUI EN CASO QUE LA VERSIÓN MEJORDA FALLE 
    ...
    def register(self):
        ...
        if request.method == "POST":
            if not form.validate_on_submit():
                ...
                flash("Error con el formulario. Por favor intente más tarde.")
                print(f"Errores en el formulario: {errores}", "danger")
        ...
        try:
            ...
            # Si existe, avisar y no continuar
            if user_exists:
                flash("El documento o correo ya está registrado.", "warning")
                return render_template("home/registro.html", form=form)

            # Hash de contraseña en Python (SALT + PEPPER)
            salt = secrets.token_bytes(16)
            hash_password = hashlib.sha256(
                salt + form.password.data.encode() + self.PEPPER.encode()
            ).digest()
...
from app.security.hash import generar_salt, hashear_contrasena, verificar_contrasena
...
class Login:
    ...
    def _validar_usuario(self, username, password, salt):
        """Valida si las credenciales ingresadas son válidas."""
        pepper = Config.PEPPER
        ...
        try:
            ...
            hash_password = hashlib.sha256(salt + password + pepper).digest()
    ...
    def register(self):
        ...
        if request.method == "POST":
            ...
            try:
                ...
                # Hash de contraseña (SALT + PASSWORD + PEPPER)
                salt          = secrets.token_bytes(16)
                hash_password = hashlib.sha256(
                    salt + form.password.data.encode() + self.PEPPER.encode()
                ).digest()
...
class RecuperarContrasena:
    ...
    def nueva_contrasena(self):
        ...
        nuevo_hash  = self._hash_password(form.password.data, nuevo_salt)
    ...
    def _hash_password(self, password: str, salt: bytes) -> bytes:
        """Genera hash SHA-256 con salt + pepper."""
        pepper = self.PEPPER.encode() if isinstance(self.PEPPER, str) else self.PEPPER
        return hashlib.sha256(salt + password.encode() + pepper).digest()
